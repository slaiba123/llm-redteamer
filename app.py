...
from ui.results import render_status_bar, render_results

load_dotenv()

# ── NODE → STEP MAPPING (matches graph/graph.py add_node names) ───────────────
# planner done  → step 1 active (executor blinking)
# executor done → step 2 active (judge blinking)
# judge done    → step 3 active (reporter blinking)
# reporter done → step 4 (all green)
NODE_TO_STEP = {
    "planner": 1,
    "executor": 2,
    "judge": 3,
...
<div class="topbar">
    <span class="topbar-badge">Security</span>
    <span class="topbar-title">LLM Red-Teaming Agent</span>
    <span class="topbar-sub">LangGraph · {config["provider"]} · LangSmith</span>
</div>
""", unsafe_allow_html=True)
...
if not config["run"]:
    ...
    if not config["system_prompt"] or not config["system_prompt"].strip():
        st.error(
            "No system prompt provided. Select a preset or write a custom one in the sidebar."
        )
        ...
        try:
            ...
            # IMPORTANT:
            # Use dict() so we keep a cumulative full state
            # instead of overwriting with partial node outputs.
            ...
            for chunk in graph.stream(initial_state):
                ...
                for node_name, state_snapshot in chunk.items():
                    ...
                    # Update status UI
                    completed_step = NODE_TO_STEP.get(node_name, 0)

                    status_placeholder.markdown(
                        render_status_bar(completed_step),
                        unsafe_allow_html=True
                    )

                    # Merge incremental updates into final state
                    if state_snapshot:
                        final_state.update(state_snapshot)

            # Ensure all steps show green after completion
            status_placeholder.markdown(
                render_status_bar(4),
                unsafe_allow_html=True
            )
            ...
            # Defensive defaults
            final_state.setdefault("attack_prompts", [])
            final_state.setdefault("attack_results", [])
            final_state.setdefault("judged_results", [])
            final_state.setdefault("report", "")
